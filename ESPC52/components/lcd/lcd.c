...
esp_err_t lcd_init(void)
{
    ...
    /* STEP6：打开 LCD 面板显示输出。
     * 功能：
     *     让 ST7789 从显示关闭状态切换到显示打开状态；背光未接 MCU，因此本步骤只控制面板显示命令。
     * 调用方法：
     *     lcd_init();     // 内部执行本步骤，并打印 STEP6 的开始日志和返回值。
     */
    ESP_LOGI(TAG, "STEP6 esp_lcd_panel_disp_on_off begin");
    ret = esp_lcd_panel_disp_on_off(s_lcd_panel, true);
    ESP_LOGI(TAG, "STEP6 esp_lcd_panel_disp_on_off ret=%d (%s)", (int)ret, esp_err_to_name(ret));
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "LCD 打开显示失败: %s", esp_err_to_name(ret));
        return ret;
    }
