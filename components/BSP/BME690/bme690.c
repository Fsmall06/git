...
static esp_err_t bme690_check_ready(void)
{
    ...
    {
        ESP_LOGI(TAG, "BME690 使用的 I2C 总线未初始化，请先调用 bme690_init()");
...
static esp_err_t bme690_write_reg(uint8_t reg_addr, uint8_t data)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGI(TAG, "写寄存器失败,reg: 0x%02X, data: 0x%02X, ret: %d", reg_addr, data, ret);
    }
...
static esp_err_t bme690_read_regs(uint8_t reg_addr, uint8_t *data, size_t len)
{
    if ((data == NULL) || (len == 0))
    {
        ESP_LOGI(TAG, "读寄存器参数错误,data: %p, len: %d", (void *)data, (int)len);
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGI(TAG, "读寄存器失败,reg: 0x%02X, len: %d, ret: %d", reg_addr, (int)len, ret);
    }
...
static esp_err_t bme690_soft_reset(void)
{
    ...
    ESP_LOGI(TAG, "BME690 soft reset 完成，等待 %d ms", BME690_RESET_WAIT_MS);
    return ESP_OK;
}
...
static esp_err_t bme690_read_chip_id(void)
{
    ...
    ESP_LOGI(TAG, "读取 chip id 成功,chip id: 0x%02X", s_chip_id);

#if BME690_CHECK_CHIP_ID_ENABLE
    if (s_chip_id != BME690_EXPECTED_CHIP_ID)
    {
        ESP_LOGI(TAG, "chip id 不匹配，期望: 0x%02X, 实际: 0x%02X",
...
static esp_err_t bme690_read_calibration(void)
{
    ...
    ESP_LOGI(TAG, "校准参数读取完成,variant id: 0x%02X, high_gas:%d, heat_range: %u, heat_val: %d, range_switching_error: %d",
             s_calib.variant_id,
...
static esp_err_t bme690_config_sensor(void)
{
    ...
    ESP_LOGI(TAG, "BME690 配置完成,osrs_t:%u, osrs_p:%u, osrs_h:%u, filter:%u",
...
esp_err_t bme690_init(void)
{
    if (s_inited)
    {
        ESP_LOGI(TAG, "BME690 已初始化,chip id: 0x%02X", s_chip_id);
        return ESP_OK;
    }

    ESP_LOGI(TAG, "开始初始化 BME690,I2C port: %d", BME690_IIC_PORT);
    ESP_LOGI(TAG, "BME690 当前使用的 I2C 地址: 0x%02X", BME690_I2C_ADDR);

    i2c_obj_t iic_obj = iic_init(BME690_IIC_PORT);
    if (iic_obj.init_flag != ESP_OK)
    {
        ESP_LOGI(TAG, "BME690 初始化失败,I2C 初始化 ret: %d", iic_obj.init_flag);
...
esp_err_t bme690_read(bme690_data_t *data)
{
    if (data == NULL)
    {
        ESP_LOGI(TAG, "bme690_read 参数错误,data 不能为空");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (!raw.new_data)
    {
        ESP_LOGI(TAG, "BME690 读取超时，等待 %lu ms 后仍无新数据", (unsigned long)waited_ms);
